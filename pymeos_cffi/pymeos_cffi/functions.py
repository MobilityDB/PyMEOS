...
def pg_date_mi(d1: 'DateADT', d2: 'DateADT') -> 'Interval *':
    d1_converted = _ffi.cast('DateADT', d1)
    d2_converted = _ffi.cast('DateADT', d2)
    result = _lib.pg_date_mi(d1_converted, d2_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def pg_date_mi_int(d: 'DateADT', days: int) -> 'DateADT':
    d_converted = _ffi.cast('DateADT', d)
    days_converted = _ffi.cast('int32', days)
    result = _lib.pg_date_mi_int(d_converted, days_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def pg_date_pl_int(d: 'DateADT', days: int) -> 'DateADT':
    d_converted = _ffi.cast('DateADT', d)
    days_converted = _ffi.cast('int32', days)
    result = _lib.pg_date_pl_int(d_converted, days_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def pg_date_timestamptz(d: 'DateADT') -> 'TimestampTz':
    d_converted = _ffi.cast('DateADT', d)
    result = _lib.pg_date_timestamptz(d_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def pg_timestamptz_date(t: int) -> 'DateADT':
    t_converted = _ffi.cast('TimestampTz', t)
    result = _lib.pg_timestamptz_date(t_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def pg_to_timestamptz(date_txt: str, fmt: str) -> 'TimestampTz':
    date_txt_converted = cstring2text(date_txt)
    fmt_converted = cstring2text(fmt)
    result = _lib.pg_to_timestamptz(date_txt_converted, fmt_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def spanset_end_span(ss: 'const SpanSet *') -> 'const Span *':
    ss_converted = _ffi.cast('const SpanSet *', ss)
...
def spanset_span_n(ss: 'const SpanSet *', i: int) -> 'const Span *':
    ss_converted = _ffi.cast('const SpanSet *', ss)
...
def spanset_start_span(ss: 'const SpanSet *') -> 'const Span *':
    ss_converted = _ffi.cast('const SpanSet *', ss)
...
def timestamptz_extent_transfn(s: 'Span *', t: int) -> 'Span *':
    s_converted = _ffi.cast('Span *', s)
    t_converted = _ffi.cast('TimestampTz', t)
    result = _lib.timestamptz_extent_transfn(s_converted, t_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def stbox_make(hasx: bool, hasz: bool, geodetic: bool, srid: int, xmin: float, xmax: float, ymin: float, ymax: float, zmin: float, zmax: float, s: 'const Span *') -> 'STBox *':
    srid_converted = _ffi.cast('int32', srid)
    s_converted = _ffi.cast('const Span *', s)
...
def tstzspan_to_tbox(s: 'const Span *') -> 'TBox *':
    s_converted = _ffi.cast('const Span *', s)
    result = _lib.tstzspan_to_tbox(s_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def tstzspanset_to_tbox(ss: 'const SpanSet *') -> 'TBox *':
    ss_converted = _ffi.cast('const SpanSet *', ss)
    result = _lib.tstzspanset_to_tbox(ss_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def temporal_to_tsequence(temp: 'const Temporal *', interp: 'interpType') -> 'TSequence *':
    temp_converted = _ffi.cast('const Temporal *', temp)
    interp_converted = _ffi.cast('interpType', interp)
...
def temporal_to_tsequenceset(temp: 'const Temporal *', interp: 'interpType') -> 'TSequenceSet *':
    temp_converted = _ffi.cast('const Temporal *', temp)
    interp_converted = _ffi.cast('interpType', interp)
...
def tpoint_at_geom_time(temp: 'const Temporal *', gs: 'const GSERIALIZED *', zspan: "Optional['const Span *']", period: 'const Span *') -> 'Temporal *':
    temp_converted = _ffi.cast('const Temporal *', temp)
    gs_converted = _ffi.cast('const GSERIALIZED *', gs)
    zspan_converted = _ffi.cast('const Span *', zspan) if zspan is not None else _ffi.NULL
...
def tpoint_minus_geom_time(temp: 'const Temporal *', gs: 'const GSERIALIZED *', zspan: "Optional['const Span *']", period: 'const Span *') -> 'Temporal *':
    temp_converted = _ffi.cast('const Temporal *', temp)
    gs_converted = _ffi.cast('const GSERIALIZED *', gs)
    zspan_converted = _ffi.cast('const Span *', zspan) if zspan is not None else _ffi.NULL
    period_converted = _ffi.cast('const Span *', period)
...
def tstzspan_bucket_list(bounds: 'const Span *', duration: 'const Interval *', origin: int) -> "Tuple['Span *', 'int']":
    bounds_converted = _ffi.cast('const Span *', bounds)
    duration_converted = _ffi.cast('const Interval *', duration)
    origin_converted = _ffi.cast('TimestampTz', origin)
    count = _ffi.new('int *')
    result = _lib.tstzspan_bucket_list(bounds_converted, duration_converted, origin_converted, count)
    _check_error()
    return result if result != _ffi.NULL else None, count[0]


def stbox_tile(point: 'GSERIALIZED *', t: int, xsize: float, ysize: float, zsize: float, duration: 'Interval *', sorigin: 'GSERIALIZED *', torigin: int, hast: bool) -> 'STBox *':
    ...
    torigin_converted = _ffi.cast('TimestampTz', torigin)
    result = _lib.stbox_tile(point_converted, t_converted, xsize, ysize, zsize, duration_converted, sorigin_converted, torigin_converted, hast)
    _check_error()
    return result if result != _ffi.NULL else None
...
def floatspan_rnd(span: 'const Span *', size: 'Datum') -> 'Span *':
    span_converted = _ffi.cast('const Span *', span)
    size_converted = _ffi.cast('Datum', size)
    out_result = _ffi.new('Span *')
    _lib.floatspan_rnd(span_converted, size_converted, out_result)
    _check_error()
    return out_result if out_result!= _ffi.NULL else None
...
def set_cmp1(s1: 'const Set *', s2: 'const Set *') -> 'int':
    s1_converted = _ffi.cast('const Set *', s1)
    s2_converted = _ffi.cast('const Set *', s2)
    result = _lib.set_cmp1(s1_converted, s2_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def set_eq1(s1: 'const Set *', s2: 'const Set *') -> 'bool':
    s1_converted = _ffi.cast('const Set *', s1)
    s2_converted = _ffi.cast('const Set *', s2)
    result = _lib.set_eq1(s1_converted, s2_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def span_cmp1(s1: 'const Span *', s2: 'const Span *') -> 'int':
    s1_converted = _ffi.cast('const Span *', s1)
    s2_converted = _ffi.cast('const Span *', s2)
    result = _lib.span_cmp1(s1_converted, s2_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def span_eq1(s1: 'const Span *', s2: 'const Span *') -> 'bool':
    s1_converted = _ffi.cast('const Span *', s1)
    s2_converted = _ffi.cast('const Span *', s2)
    result = _lib.span_eq1(s1_converted, s2_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def spanset_cmp1(ss1: 'const SpanSet *', ss2: 'const SpanSet *') -> 'int':
    ss1_converted = _ffi.cast('const SpanSet *', ss1)
    ss2_converted = _ffi.cast('const SpanSet *', ss2)
    result = _lib.spanset_cmp1(ss1_converted, ss2_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def spanset_eq1(ss1: 'const SpanSet *', ss2: 'const SpanSet *') -> 'bool':
    ss1_converted = _ffi.cast('const SpanSet *', ss1)
    ss2_converted = _ffi.cast('const SpanSet *', ss2)
    result = _lib.spanset_eq1(ss1_converted, ss2_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def over_adj_span_span(s1: 'const Span *', s2: 'const Span *') -> 'bool':
    s1_converted = _ffi.cast('const Span *', s1)
    s2_converted = _ffi.cast('const Span *', s2)
    result = _lib.over_adj_span_span(s1_converted, s2_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def left_notadj_span_span(s1: 'const Span *', s2: 'const Span *') -> 'bool':
    s1_converted = _ffi.cast('const Span *', s1)
    s2_converted = _ffi.cast('const Span *', s2)
    result = _lib.left_notadj_span_span(s1_converted, s2_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def intersection_set_value(s: 'const Set *', d: 'Datum', basetype: 'meosType') -> 'Set *':
    s_converted = _ffi.cast('const Set *', s)
    d_converted = _ffi.cast('Datum', d)
    basetype_converted = _ffi.cast('meosType', basetype)
    result = _lib.intersection_set_value(s_converted, d_converted, basetype_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def intersection_span_value(s: 'const Span *', d: 'Datum', basetype: 'meosType') -> 'Span *':
    s_converted = _ffi.cast('const Span *', s)
    d_converted = _ffi.cast('Datum', d)
    basetype_converted = _ffi.cast('meosType', basetype)
    result = _lib.intersection_span_value(s_converted, d_converted, basetype_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def intersection_spanset_value(ss: 'const SpanSet *', d: 'Datum', basetype: 'meosType') -> 'SpanSet *':
    ss_converted = _ffi.cast('const SpanSet *', ss)
    d_converted = _ffi.cast('Datum', d)
    basetype_converted = _ffi.cast('meosType', basetype)
    result = _lib.intersection_spanset_value(ss_converted, d_converted, basetype_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def minus_value_set(d: 'Datum', basetype: 'meosType', s: 'const Set *') -> 'Set *':
    d_converted = _ffi.cast('Datum', d)
    basetype_converted = _ffi.cast('meosType', basetype)
    s_converted = _ffi.cast('const Set *', s)
    result = _lib.minus_value_set(d_converted, basetype_converted, s_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def minus_value_span(d: 'Datum', basetype: 'meosType', s: 'const Span *') -> 'SpanSet *':
    d_converted = _ffi.cast('Datum', d)
    basetype_converted = _ffi.cast('meosType', basetype)
    s_converted = _ffi.cast('const Span *', s)
    result = _lib.minus_value_span(d_converted, basetype_converted, s_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def minus_value_spanset(d: 'Datum', basetype: 'meosType', ss: 'const SpanSet *') -> 'SpanSet *':
    d_converted = _ffi.cast('Datum', d)
    basetype_converted = _ffi.cast('meosType', basetype)
    ss_converted = _ffi.cast('const SpanSet *', ss)
    result = _lib.minus_value_spanset(d_converted, basetype_converted, ss_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def dist_set_set(s1: 'const Set *', s2: 'const Set *') -> 'double':
    s1_converted = _ffi.cast('const Set *', s1)
    s2_converted = _ffi.cast('const Set *', s2)
...
def dist_span_span(s1: 'const Span *', s2: 'const Span *') -> 'double':
    s1_converted = _ffi.cast('const Span *', s1)
    s2_converted = _ffi.cast('const Span *', s2)
...
def tbox_shift_scale_value(box: 'const TBox *', shift: 'Datum', width: 'Datum', basetype: 'meosType', hasshift: bool, haswidth: bool) -> 'TBox *':
    box_converted = _ffi.cast('const TBox *', box)
    shift_converted = _ffi.cast('Datum', shift)
    width_converted = _ffi.cast('Datum', width)
    basetype_converted = _ffi.cast('meosType', basetype)
    result = _lib.tbox_shift_scale_value(box_converted, shift_converted, width_converted, basetype_converted, hasshift, haswidth)
    _check_error()
    return result if result != _ffi.NULL else None
...
def tinstant_as_mfjson(inst: 'const TInstant *', with_bbox: bool, precision: int, srs: str) -> str:
    inst_converted = _ffi.cast('const TInstant *', inst)
    srs_converted = srs.encode('utf-8')
    result = _lib.tinstant_as_mfjson(inst_converted, with_bbox, precision, srs_converted)
    _check_error()
    result = _ffi.string(result).decode('utf-8')
    return result if result != _ffi.NULL else None
...
def tsequence_as_mfjson(seq: 'const TSequence *', with_bbox: bool, precision: int, srs: str) -> str:
    seq_converted = _ffi.cast('const TSequence *', seq)
    srs_converted = srs.encode('utf-8')
    result = _lib.tsequence_as_mfjson(seq_converted, with_bbox, precision, srs_converted)
    _check_error()
    result = _ffi.string(result).decode('utf-8')
    return result if result != _ffi.NULL else None
...
def tsequenceset_as_mfjson(ss: 'const TSequenceSet *', with_bbox: bool, precision: int, srs: str) -> str:
    ss_converted = _ffi.cast('const TSequenceSet *', ss)
    srs_converted = srs.encode('utf-8')
    result = _lib.tsequenceset_as_mfjson(ss_converted, with_bbox, precision, srs_converted)
    _check_error()
    result = _ffi.string(result).decode('utf-8')
    return result if result != _ffi.NULL else None
...
def tsequence_at_tstzspan(seq: 'const TSequence *', s: 'const Span *') -> 'TSequence *':
    seq_converted = _ffi.cast('const TSequence *', seq)
    s_converted = _ffi.cast('const Span *', s)
    result = _lib.tsequence_at_tstzspan(seq_converted, s_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def tsequence_restrict_timestamptz(seq: 'const TSequence *', t: int, atfunc: bool) -> 'TInstant *':
    seq_converted = _ffi.cast('const TSequence *', seq)
    t_converted = _ffi.cast('TimestampTz', t)
    result = _lib.tsequence_restrict_timestamptz(seq_converted, t_converted, atfunc)
    _check_error()
    return result if result != _ffi.NULL else None


def tsequence_restrict_tstzset(seq: 'const TSequence *', s: 'const Set *', atfunc: bool) -> 'TSequence *':
    seq_converted = _ffi.cast('const TSequence *', seq)
    s_converted = _ffi.cast('const Set *', s)
    result = _lib.tsequence_restrict_tstzset(seq_converted, s_converted, atfunc)
    _check_error()
    return result if result != _ffi.NULL else None
...
def tsequence_restrict_values(seq: 'const TSequence *', s: 'const Set *', atfunc: bool) -> 'TSequenceSet *':
    seq_converted = _ffi.cast('const TSequence *', seq)
    s_converted = _ffi.cast('const Set *', s)
    result = _lib.tsequence_restrict_values(seq_converted, s_converted, atfunc)
    _check_error()
    return result if result != _ffi.NULL else None
...
def tfloat_wmax_transfn(state: 'SkipList *', temp: 'const Temporal *', interval: 'const Interval *') -> 'SkipList *':
    state_converted = _ffi.cast('SkipList *', state)
    temp_converted = _ffi.cast('const Temporal *', temp)
    interval_converted = _ffi.cast('const Interval *', interval)
    result = _lib.tfloat_wmax_transfn(state_converted, temp_converted, interval_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def tfloat_wmin_transfn(state: 'SkipList *', temp: 'const Temporal *', interval: 'const Interval *') -> 'SkipList *':
    state_converted = _ffi.cast('SkipList *', state)
    temp_converted = _ffi.cast('const Temporal *', temp)
    interval_converted = _ffi.cast('const Interval *', interval)
    result = _lib.tfloat_wmin_transfn(state_converted, temp_converted, interval_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def tfloat_wsum_transfn(state: 'SkipList *', temp: 'const Temporal *', interval: 'const Interval *') -> 'SkipList *':
    state_converted = _ffi.cast('SkipList *', state)
    temp_converted = _ffi.cast('const Temporal *', temp)
    interval_converted = _ffi.cast('const Interval *', interval)
    result = _lib.tfloat_wsum_transfn(state_converted, temp_converted, interval_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def tint_wmax_transfn(state: 'SkipList *', temp: 'const Temporal *', interval: 'const Interval *') -> 'SkipList *':
    state_converted = _ffi.cast('SkipList *', state)
    temp_converted = _ffi.cast('const Temporal *', temp)
    interval_converted = _ffi.cast('const Interval *', interval)
    result = _lib.tint_wmax_transfn(state_converted, temp_converted, interval_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def tint_wmin_transfn(state: 'SkipList *', temp: 'const Temporal *', interval: 'const Interval *') -> 'SkipList *':
    state_converted = _ffi.cast('SkipList *', state)
    temp_converted = _ffi.cast('const Temporal *', temp)
    interval_converted = _ffi.cast('const Interval *', interval)
    result = _lib.tint_wmin_transfn(state_converted, temp_converted, interval_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def tint_wsum_transfn(state: 'SkipList *', temp: 'const Temporal *', interval: 'const Interval *') -> 'SkipList *':
    state_converted = _ffi.cast('SkipList *', state)
    temp_converted = _ffi.cast('const Temporal *', temp)
    interval_converted = _ffi.cast('const Interval *', interval)
    result = _lib.tint_wsum_transfn(state_converted, temp_converted, interval_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def tnumber_wavg_transfn(state: 'SkipList *', temp: 'const Temporal *', interval: 'const Interval *') -> 'SkipList *':
    state_converted = _ffi.cast('SkipList *', state)
    temp_converted = _ffi.cast('const Temporal *', temp)
    interval_converted = _ffi.cast('const Interval *', interval)
    result = _lib.tnumber_wavg_transfn(state_converted, temp_converted, interval_converted)
    _check_error()
    return result if result != _ffi.NULL else None
