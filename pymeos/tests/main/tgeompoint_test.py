...
from pymeos import TBool, TBoolInst, TBoolSeq, TBoolSeqSet, \
    TFloat, TFloatInst, TFloatSeq, TFloatSeqSet, TGeomPoint, \
    TGeomPointInst, TGeomPointSeq, TGeomPointSeqSet, \
    TGeogPointInst, TGeogPointSeq, TGeogPointSeqSet, \
    TInterpolation, TimestampSet, Period, PeriodSet, STBox
...
class TestTGeomPointOutputs(TestTGeomPoint):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (tpds, '{"type":"MultiPoint","bbox":[1.00,1.00,2.00,2.00],"coordinates":[[1,1],[2,2]]}'),
            (tps, '{"type":"LineString","bbox":[1.00,1.00,2.00,2.00],"coordinates":[[1,1],[2,2]]}'),
            (tpss, '{"type":"GeometryCollection","bbox":[1.00,1.00,2.00,2.00],'
...
class TestTGeomPointTemporalAccessors(TestTGeomPoint):
    ...
    def test_sequenceset_sequence_functions(self):
        ...
        assert tpss1.sequences() == [TGeomPointSeq('[Point(1 1)@2019-09-01, Point(2 2)@2019-09-02]'),
            TGeomPointSeq('[Point(1 1)@2019-09-03, Point(1 1)@2019-09-05]'), 
...
class TestTGeomPointTPointAccessors(TestTGeomPoint):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (tpds, []),
            (tps, [STBox('STBOX XT(((1,1),(2,2)),[2019-09-01, 2019-09-02])')]),
            (tpss, [STBox('STBOX XT(((1,1),(2,2)),[2019-09-01, 2019-09-02])'),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (tpss, TFloatSeqSet('{[45@2019-09-01,45@2019-09-02],[45@2019-09-03,45@2019-09-05]}')),
        ],
        ids=['Instant', 'Discrete Sequence', 'Sequence', 'SequenceSet']
    )
...
class TestTGeomPointConversions(TestTGeomPoint):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (tpss, TGeogPointSeqSet('{[Point(1 1)@2019-09-01, Point(2 2)@2019-09-02],'
...
class TestTGeomPointTransformations(TestTGeomPoint):
    ...
    tpss_s = TGeomPointSeqSet('{[Point(1 1)@2019-09-01, Point(1 1)@2019-09-02],'
        '[Point(2 2)@2019-09-03, Point(2 2)@2019-09-05]}')
    tps_l = TGeomPointSeq('Interp=Step;[Point(1 1)@2019-09-01, Point(2 2)@2019-09-02]')
    tpss_l = TGeomPointSeqSet('Interp=Step;{[Point(1 1)@2019-09-01, Point(2 2)@2019-09-02],'
        '[Point(1 1)@2019-09-03, Point(1 1)@2019-09-05]}')
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (tpss_s, TInterpolation.STEPWISE,
                TGeomPointSeqSet('Interp=Step;{[Point(1 1)@2019-09-01, Point(1 1)@2019-09-02],'
                '[Point(2 2)@2019-09-03, Point(2 2)@2019-09-05]}')),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (TGeomPointSeq('{Point(1.123456789 1.123456789)@2019-09-01,'
                'Point(2.123456789 2.123456789)@2019-09-02}'), 
                TGeomPointSeq('{Point(1.12 1.12)@2019-09-01,Point(2.12 2.12)@2019-09-02}')),
            (TGeomPointSeq('[Point(1.123456789 1.123456789)@2019-09-01,'
                'Point(2.123456789 2.123456789)@2019-09-02]'), 
                TGeomPointSeq('[Point(1.12 1.12)@2019-09-01,Point(2.12 2.12)@2019-09-02]')),
            (TGeomPointSeqSet('{[Point(1.123456789 1.123456789)@2019-09-01,'
                'Point(2.123456789 2.123456789)@2019-09-02],' 
...
class TestTGeomPointTemporalSpatialOperations(TestTGeomPoint):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (tpi, TGeomPointInst('Point(1 1)@2019-09-01'), TBoolInst('True@2019-09-01')),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (tpss, TBoolSeqSet('{[True@2019-09-01, True@2019-09-02],'
                '[True@2019-09-03, True@2019-09-05]}')),
        ],
        ids=['Instant', 'Discrete Sequence', 'Sequence', 'SequenceSet']
    )
...
class TestTGeomPointDistanceOperations(TestTGeomPoint):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (tpss, Point(1,1), TFloatSeqSet('{[0@2019-09-01, 1.414@2019-09-02],'
                '[0@2019-09-03, 0@2019-09-05]}')),

            (tpi, STBox('STBOX X((1,1),(1,1))'), TFloatInst('0@2019-09-01')),
            (tpds, STBox('STBOX X((1,1),(1,1))'), TFloatSeq('{0@2019-09-01, 1.414@2019-09-02}')),
            (tps, STBox('STBOX X((1,1),(1,1))'), TFloatSeq('[0@2019-09-01, 1.414@2019-09-02]')),
            (tpss, STBox('STBOX X((1,1),(1,1))'), TFloatSeqSet('{[0@2019-09-01, 1.414@2019-09-02],'
                '[0@2019-09-03, 0@2019-09-05]}')),
            ...
            (tpss,
                TGeomPointSeqSet('{[Point(1 1)@2019-09-01, Point(1 1)@2019-09-02],'
                '[Point(1 1)@2019-09-03, Point(1 1)@2019-09-05]}'),
                TFloatSeqSet('{[0@2019-09-01, 1.414@2019-09-02],'
                '[0@2019-09-03, 0@2019-09-05]}')),
        ],
        ids=['Instant Geo', 'Discrete Sequence Geo', 'Sequence Geo', 'SequenceSet Geo',
